...
public class Principal {
	...
	public static void main(String[] args) {
		...
		//Executa o menu e os métodos da main enquanto o usuário não digita 7 e 
	...
	public static void cadastroCliente(Cliente objCliente) {
		...
		do {
			...
			//Se for menor que 0, mostra a mensagem da linha 141
			if (quantidade == 0) {
				break;
		...
		//Definição dos atributos nome, endereço e telefone que serão armazenados no objeto 
		//da classe Cliente
		for (int i = 0; i < quantidade; i++) {
			...
			//Adicionando o objeto criado na classe ConjuntoClientes que contém o ArrayList
	...
	public static void buscaCliente(Cliente objCliente) {
		...
		//Verifica se há cliente cadastrado no sistema, se não houver, será exibida 
		//a mensagem das linhas 235-237 e o restante do método nem será executado
		...
		if (temClientes) {
			...
			//Busca o nome do cliente na classe ConjuntoClientes e resgata do ArrayList apenas
			//o objeto com o mesmo nome digitado pelo usuário
			objCliente = ConjuntoClientes.pesquisarCliente(nome);
			
			//Lista os dados do objeto cliente resgatado, se o nome digitado não bater com
			//algum nome cadastrado, mostra a mensagem das linhas 230-232
			if (objCliente != null) {
				System.out.println("=========================================================================================================");
				...
				if (opcao == 'S' || opcao == 's') {
					...
					//Atualização dos dados do objeto do tipo Cliente
	...
	public static void cadastroProduto(Produto objProduto) {
		...
		do {
			...
			quantidade = LerDados.lerInt(quantidade);
			//Se o número digitado for 0, partimos do pressuposto que o usuário quer voltar ao menu
			//Se for menor que 0, mostra a mensagem da linha 259
			if (quantidade == 0) {
				break;
			} else if (quantidade < 0){
				System.out.println("Digite um número maior que 0!");
		...
		for (int i = 0; i < quantidade; i++) {
			...
			//Adicionando o objeto criado na classe ConjuntoProdutos que contém o ArrayList
	...
	public static void buscaProduto(Produto objProduto) {
		...
		//Verifica se há produto cadastrado no sistema, se não houver, será exibida 
		//a mensagem das linhas 366-368
		...
		if (temProdutos) {
			...
			//Busca o nome do produto na classe ConjuntoProdutos e resgata apenas o objeto 
			//com mesmo nome digitado pelo usuário
			objProduto = ConjuntoProdutos.pesquisarProduto(nome);
			
			//Lista os dados do objeto produto resgatado, se o nome digitado não bater com
			//algum nome cadastrado, mostra a mensagem das linhas 361-363
			if (objProduto != null) {
				System.out.println("=========================================================================================================");
				...
				if (opcao == 'S' || opcao == 's') {
					...
					//Atualização dos dados do objeto do tipo Produto
	...
	public static void cadastroDeVenda(Produto objProduto, Cliente objCliente) {
		...
		//Esse método só será executado se houver cliente E produto cadastrado no sistema
		//Caso contrário, mostra a mensagem das linhas 472-474
		if (temClientes && temProdutos) {
			//Lista os nomes dos clientes
			System.out.println("===================================================================================================");
			ConjuntoClientes.imprimeNomes();
			
			//Pede para o usuário digitar o nome a ser pesquisado
			System.out.println("===================================================================================================");
			System.out.print(">>>Digite o nome do cliente: ");
			nomeCliente = LerDados.lerString();
			
			//O nome do cliente será pesquisado e comparado com algum cliente que já exista 
			//no sistema. O objeto contendo os atributos é resgatado aqui
			objCliente = ConjuntoClientes.pesquisarCliente(nomeCliente);
			
			//Se o nome digitado bater com algum nome já no sistema, significa que o objCliente 
			//é diferente de null e podemos prosseguir.
			//Caso contrário o programa retorna a mensagem das linhas 466-469
			if (objCliente != null) {
				//Lista os nomes dos produtos
				System.out.println("===================================================================================================");
				ConjuntoProdutos.imprimeNomes();
				...
				//O nome do produto será pesquisado e comparado com algum produto que já exista 
				//no sistema. O objeto contendo os atributos é resgatado aqui
				objProduto = ConjuntoProdutos.pesquisarProduto(nomeProduto);
				
				//Se o nome digitado bater com algum nome já no sistema, significa que o objProduto 
				//é diferente de null e podemos prosseguir.
				//Caso contrário, o programa retorna a mesagem das linhas 460-463
				if (objProduto != null) {
					do {				
						...
						//Se a quantidade digitada for negativa, o programa continuará perguntando 
						//a quantidade. Se for 0, entendemos que o usuário quer voltar ao menu
						if (quantidade == 0) {
							break;
						} else {
							if (quantidade < 0) {
								...
								//Se a quantidade digitada for maior que a quantidade de produto no 
								//estoque será exibida uma mensagem avisando que o valor digitado é 
								//maior que a quantidade em estoque do produto e o programa continuará
								//perguntando a quantidade. Caso contrário, o programa irá retirar a 
								//quantidade digitada do produto do estoque
					...
					//O loop permanecerá até o usuário digitar uma quantidade maior que zero e um 
					//valor que não ultrapasse a quantidade em estoque do produto
	...
	public static void mostrarProdutosEmEstoque() {
		//Mostra os produtos no AarrayList com suas respectivas quantidades no estoque
		System.out.println("===================================================================================================");
